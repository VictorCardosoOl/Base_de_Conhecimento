...
export const AdminPage: React.FC = () => {
    ...
    // Gestão de Validade: Detecta artigos com mais de 6 ou 12 meses sem revisão
    ...
    return (
        <div className="max-w-screen-2xl 3xl:max-w-[2100px] mx-auto space-y-8 2xl:space-y-12 p-4 2xl:p-8">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-serif text-text-main">Painel Administrativo & Governança</h1>
                    <p className="text-text-muted mt-1">Gestão de artigos, auditoria de normas e inteligência de busca</p>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-selection/30 border-b border-border text-sm text-text-muted uppercase tracking-wider">
                                    ...
                                    <th className="p-4 font-medium">Última Revisão</th>
            ...
            {/* Conteúdo da Aba 2: Zero-Result Searches */}
            {activeTab === 'analytics' && (
                <div className="space-y-6">
                    <div className="p-6 rounded-xl bg-blue-500/5 border border-blue-500/20 text-text-main">
                        <h3 className="text-sm font-semibold flex items-center gap-2">
                            ...
                            Gaps de Conteúdo Corporativo (Consultas Sem Resultado)
                        </h3>
                        <p className="text-xs text-text-muted mt-1">
                            Estes são os termos técnicos buscados por colaboradores e clientes que retornaram zero artigos. Use esta lista para priorizar a criação de novos procedimentos em Markdown.
                        </p>
                    </div>
            ...
            {/* Conteúdo da Aba 3: Auditoria de Validade */}
            {activeTab === 'audit' && (
                <div className="space-y-6">
                    <div className="p-6 rounded-xl bg-amber-500/5 border border-amber-500/20 text-text-main">
                        <h3 className="text-sm font-semibold flex items-center gap-2">
                            <Clock size={16} className="text-amber-600 dark:text-amber-400" />
                            Controle de Vencimento e Recertificação Técnica de SST
                        </h3>
                        <p className="text-xs text-text-muted mt-1">
                            Diretrizes normativas e previdenciárias exigem revisão periódica de 6 ou 12 meses. Artigos com alerta devem ser revalidados pelo responsável técnico.
                        </p>
                    </div>
                            ...
                            <div key={idx} className="p-5 rounded-xl border border-border bg-bg-island space-y-3">
                                <div className="flex items-start justify-between gap-4">
                                    ...
                                    <span className={`px-2 py-1 rounded text-xs font-bold shrink-0 ${alert.isExpired ? 'bg-red-500/10 text-red-600 border border-red-500/20' : 'bg-amber-500/10 text-amber-600 border border-amber-500/20'}`}>
                                        {alert.isExpired ? 'Vencido' : 'Próximo do Vencimento'}
                                    </span>
                                </div>
                                <div className="text-xs text-text-muted space-y-1">
                                    <p>Última Verificação: <strong>{alert.item.lastReviewed || alert.item.date}</strong></p>
                                    <p>Periodicidade Máxima: <strong>{alert.maxValidity} meses</strong> (decorridos {alert.diffMonths} meses)</p>
                                    <p>Responsável Técnico: <strong>{alert.item.verifiedBy || 'Engenharia de SST'}</strong></p>
                                </div>
            ...
            {/* Conteúdo da Aba 4: Feedbacks e Desatualizações */}
            {activeTab === 'feedback' && (
                <div className="space-y-6">
                    <div className="p-6 rounded-xl bg-purple-500/5 border border-purple-500/20 text-text-main">
                        <h3 className="text-sm font-semibold flex items-center gap-2">
                            ...
                            Backlog de Apontamentos da Comunidade e Usuários
                        </h3>
                        <p className="text-xs text-text-muted mt-1">
                            Feedbacks enviados através dos botões de avaliação rápida nos procedimentos e formulário de "Reportar Desatualização".
                        </p>
                    </div>
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${fb.type === 'outdated_report' ? 'bg-amber-500/10 text-amber-600 border border-amber-500/20' : 'bg-emerald-500/10 text-emerald-600'}`}>
                                            {fb.type === 'outdated_report' ? 'Desatualização Reportada' : fb.type === 'useful_yes' ? 'Avaliação Positiva' : 'Avaliação Negativa'}
                                        </span>
